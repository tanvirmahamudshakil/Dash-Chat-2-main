...
class DefaultDateSeparator extends StatelessWidget {
  ...
  String _formatDateSeparator(DateTime date) {
    ...
    // return intl.DateFormat('EEE dd').format(date);
    return dateformet(date);
  }
  ...
  String dateformet(DateTime date) {
    ...
    DateTime myday = DateTime(today.year, today.month, today.day);
    DateTime msgdate = DateTime(date.year, date.month, date.day);
    int differ = myday.difference(msgdate).inDays;

    if (differ > 1 && differ < 7) {
      return intl.DateFormat("EEE dd").format(date);
